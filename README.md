...
1. 결제 모듈 추가
  * KG Inicis 모듈 붙이기 완료.
  * 사용자 주문 페이지 구현 완료.
  * 모듈에 주문 페이지의 값을 넘겨서 잘 출력되는지 확인해야한다.
  * 결제 완료시 주문 내역이 DB에 저장되어야하고
  * 적립금, 쿠폰 등을 쓴 것이 있다면 그것 또한 반영되어야하며
  * 만약 사용자가 환불을 원하는 경우, 관리자에게 선 Alert 후 확인이 되면 그 때 주문내역 삭제, 적립금 및 쿠폰 등 롤백이 되어야한다.
